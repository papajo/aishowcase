...
export default async function HomePage() {
   ...
   * 1. public/heroes/manual/{slug}.webp  — User created manually, highest priority
   * 2. public/heroes/auto/{slug}.webp    — AI-generated by scripts/generate_heroes.py
   * 3. Topic-matched SVG illustration     — Fallback from keyword-based selection
   */
  function heroFor(post: { title?: string; tags?: string[]; excerpt?: string; slug?: string }) {
    // Check for a hero image file (manual > auto)
    const slug = post.slug ?? ""
    const manualPath = path.join(process.cwd(), "public", "heroes", "manual", `${slug}.webp`)
    const autoPath = path.join(process.cwd(), "public", "heroes", "auto", `${slug}.webp`)

    if (fs.existsSync(manualPath)) {
      return (
        <Image
          src={`/heroes/manual/${slug}.webp`}
    ...
    // Fallback: topic-matched SVG illustrations
