...
export function JournalForm({ post }: JournalFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const url = post ? `/api/admin/journal/${post.id}` : "/api/admin/journal"
    ...
    <form onSubmit={handleSubmit}>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <Link href="/admin/journal" className="p-2 hover:bg-muted rounded-lg transition-colors">
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <h1 className="text-2xl font-bold tracking-tight">{post ? "Edit Entry" : "New Entry"}</h1>
        </div>
      ...
      <div className="grid gap-6 max-w-2xl">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setForm((f) => ({ ...f, title, slug: post ? f.slug : generateSlug(title) }))
                }}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {form.tags.map((tag) => (
                <span key={tag} className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-secondary text-sm">
                  {tag}
        ...
        <Card>
          <CardHeader><CardTitle>Content (MDX)</CardTitle></CardHeader>
          <CardContent>
            <textarea
              value={form.content}
